    ...
    <statusbar :mode="statusbarMode">
      ...
      <div class="tabulator-paginator">
        <div class="flex-center flex-middle flex">
          <a @click="page = 1" v-tooltip="ctrlOrCmd('left')">
            <i class="material-icons flip">double_arrow</i>
          </a>
          ...
          <a @click="page = totalPage" v-tooltip="ctrlOrCmd('right')">
      ...
      <div class="col x4 statusbar-actions flex-right">
        ...
        <div class="input_box">
          <input
            ...
            type="number"
            v-model="inputLimit"
            v-on:keyup.enter="editLimit"
          />
          <x-button
            class="btn btn-flat"
            @click="editLimit"
            v-tooltip="`Set limit and refresh table`"
          >
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      response: null,
      totalPage: 1,
      limit: parseInt(localStorage.getItem("tableLimit")) || 1000,
      inputLimit: parseInt(localStorage.getItem("tableLimit")) || 1000,
      rawTableKeys: [],
      primaryKeys: null,
      pendingChanges: {
        inserts: [],
  ...
  methods: {
    async editLimit() {
      localStorage.setItem("tableLimit", this.inputLimit);
      this.limit = parseInt(this.inputLimit);
    ...
    dataFetch(_url, _config, params) {
      ...
      const result = new Promise((resolve, reject) => {
        (async () => {
          try {
            const lengthResponse = await this.connection.getTableLength(
              this.table.name
            );
            this.totalPage = Math.ceil(lengthResponse / limit);
